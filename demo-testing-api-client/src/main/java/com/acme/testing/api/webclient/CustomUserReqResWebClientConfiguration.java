...
@Component
public class CustomUserReqResWebClientConfiguration {

	private static final Logger logger = LoggerFactory.getLogger(CustomUserReqResWebClientConfiguration.class);

	@Bean
	public WebClient customWebClient() {

		TcpClient tcpClient = TcpClient.create().option(ChannelOption.CONNECT_TIMEOUT_MILLIS, 2_000)
				.doOnConnected(connection -> connection.addHandlerLast(new ReadTimeoutHandler(2))
						.addHandlerLast(new WriteTimeoutHandler(2)));

		return WebClient.builder().baseUrl(ReqResApiConstant.ROOT_URI)
